from PyQt6.QtWidgets import QWidget, QVBoxLayout, QTableWidget, QTableWidgetItem, QHBoxLayout, QPushButton, QFileDialog, QStyle
...
class ReceivedFramesWindow(QWidget):
    def __init__(self, dbc=None):
        ...
        self.btn_link_csv.setIcon(self.style().standardIcon(QStyle.StandardPixmap.SP_DriveFDIcon))
        ...
        self.table.setHorizontalHeaderLabels([
            "ID", "Nome", "DLC", "Payload (0-7)",
            "Ultimo ricevimento", "Conteggio", "Periodo (ms)", "Dev. Std (ms)"
        ])
    ...
    def refresh_table(self):
        for frame_id, f in self._rx_buffer.items():
            ...
            self.table.setItem(row, 3, QTableWidgetItem(' '.join(f"{b:02X}" for b in f['data'])))
            last_recv = QDateTime.fromSecsSinceEpoch(int(f['last_time']))
            self.table.setItem(row, 4, QTableWidgetItem(last_recv.toString(Qt.DateFormat.ISODate)))
            self.table.setItem(row, 5, QTableWidgetItem(str(f['count'])))
            self.table.setItem(row, 6, QTableWidgetItem(f"{f['avg_period']:.1f}" if f['avg_period'] else "-"))
            self.table.setItem(row, 7, QTableWidgetItem(f"{std_dev:.1f}" if std_dev else "-"))
        ...
        if self.log_active and not self.log_paused and self.csv_writer:
            try:
                ...
                self.csv_writer.writerow([
                    timestamp, id_str, msg_name, f['dlc'], payload_str,
                    f['count'], f"{f['avg_period']:.1f}" if f['avg_period'] else "-",
    ...
    def link_csv_file(self):
        path, _ = QFileDialog.getSaveFileName(self, "Seleziona file CSV", "", "CSV Files (*.csv)")
        if path:
            self.csv_path = path
    ...
    def start_log(self): # metodo per avviare il log e aprire il file CSV
        ...
        if not self.log_active or not self.csv_file:
            ...
            self.csv_writer.writerow(["Timestamp", "ID", "Nome", "DLC", "Payload", "Conteggio", "Periodo(ms)", "Dev.Std(ms)"])
