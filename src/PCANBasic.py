...
class PCANBasic:
    ...
    def __init__(self):
        ...
        try:
            ...
            print("Exception: The PCAN-Basic DLL couldn't be loaded! Error:", e)
    ...
    def Initialize(
        ...
        HwType: TPCANType = TPCANType(0),
        IOPort: c_uint = c_uint(0),
        Interrupt: c_ushort = c_ushort(0),
    ):
        """
          Initializes a PCAN Channel
        ...
        if self.__m_dllBasic is None:
            raise Exception("PCANBasic DLL is not loaded. Cannot call CAN_Initialize.")
        try:
    ...
    def Read(self, Channel):
        ...
        """
        if self.__m_dllBasic is None:
            raise Exception("PCANBasic DLL is not loaded. Cannot call CAN_Read.")
        try:
            msg = TPCANMsg()
    ...
    #
    def ReadFD(self, Channel: TPCANHandle):
        """
          Reads a CAN message from the receive queue of a FD capable PCAN Channel
        ...
        """
        if self.__m_dllBasic is None:
            raise Exception("PCANBasic DLL is not loaded. Cannot call CAN_ReadFD.")
        try:
            msg = TPCANMsgFD()
    ...
    #
    def Write(self, Channel: TPCANHandle, MessageBuffer):
        """
          Transmits a CAN message
    ...
    #
    def WriteFD(self, Channel: TPCANHandle, MessageBuffer):
        """
          Transmits a CAN message over a FD capable PCAN Channel
    ...
    #
    def FilterMessages(
        self, Channel: TPCANHandle, FromID: c_uint, ToID: c_uint, Mode: TPCANMode
    ):
        """
          Configures the reception filter
        ...
        if self.__m_dllBasic is None:
            raise Exception(
                "PCANBasic DLL is not loaded. Cannot call CAN_FilterMessages."
            )
        try:
    ...
    #
    def GetValue(self, Channel: TPCANHandle, Parameter):
        """
          Retrieves a PCAN Channel value
    ...
    #
    def SetValue(self, Channel: TPCANHandle, Parameter, Buffer):
        """
        ...
        """
        if self.__m_dllBasic is None:
            raise Exception("PCANBasic DLL is not loaded. Cannot call CAN_SetValue.")
        try:
            if (
